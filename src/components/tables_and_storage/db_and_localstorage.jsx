...
export default function StorageComponent({onLoad, onSave, onClear}) {
  ...
  const init = new Intl.DateTimeFormat('en-US').format(xxx)

  const [entry, setEntry] = useState([])
  const [date, setDate] = useState({date:init, hidden: true})
      ...
      try {
        ...
        if(!error) {
          ...
          if(athletes.length > 0) {
            ...
            setDate({date:data.date, hidden: false})
          }
          }
        })
      } catch (error) {
  ...
  return (
    <div className="mx-5 d-flex flex-column">
      ...
      <div className="mt-5">
        ...
        <p hidden={date.hidden}>Last changes made on {date.date}</p>
